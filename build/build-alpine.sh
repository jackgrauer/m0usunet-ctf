...
# ========================================================================
# Stage everything the live VM needs.
# Copies are ordered to match the player's actual journey through the
# build -- boot first, then portal, then game shell, then in-game tools,
# then the blueprint they ultimately read.
# ========================================================================

# ---- 1. boot: PID-1 init that runs once via /etc/inittab ::sysinit ----
cp "$ROOT/build/m0use-bootstrap.sh"  "$TMP/sbin/m0use-bootstrap"
chmod 755 "$TMP/sbin/m0use-bootstrap"

# ---- 2. login: per-shell defaults sourced from /etc/profile.d --------
mkdir -p "$TMP/etc/profile.d"
cp "$ROOT/build/qol-profile.sh"      "$TMP/etc/profile.d/qol.sh"
chmod 644 "$TMP/etc/profile.d/qol.sh"

# ---- 3. portal: 4-task aptitude flow, launched from profile.d -------
cp "$ROOT/build/m0use-portal.sh"     "$TMP/usr/local/bin/m0use-portal"
chmod 755 "$TMP/usr/local/bin/m0use-portal"

# ---- 4. game shell: bash rc + the player's command-line tools -------
...
# ---- 5. fake recon tools: phase 1 nmap, phase 2 nikto, phase 3 ------
#         msfconsole, plus a fake curl that handles the Jenkins endpoint.
#         These land in /usr/local/bin which comes before /usr/bin on
#         PATH; we don't install the real packages.
